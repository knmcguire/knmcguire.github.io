...
Once the toolkit came out, we realized that many of the things we had tried to implement before were already available. The toolkit came with a fully working and controllable UR5e plus gripper (albeit a much simpler one), together with the entire framework and task board already simulated in Gazebo. We eventually decided not to go the NVIDIA Isaac route, mostly due to the difficulty of running that on everyone's machine, but also because we found out that the evaluation of the qualifying round was mainly done in Gazebo.
...
The first thing I thought about was to make a binary image of the task board by thresholding its gray scale image against its much lighter background. Then some clean up of the artifacts (like the light lines in the back) by eroding and dilating the binary image with morphological filters. Then to retrieve the edges, and fit lines on those edges with Hough lines and RANSAC filtering. Where those lines intersect, you get 4 corners, even if those corners could not be seen by the camera because it was too close by. The flat square was then projected using the camera info with the SolvePnP algorithm, to retrieve those points in 3D space and hence get the 3D pose of the task board. Of course you'd have rotation ambiguity as well, so luckily there was also a nice magenta logo that gave some clarity on that too.

On a clean, empty task board this usually works pretty well on a single camera. However, on a more cluttered task board with NIC cards in it, you'd find more squares than only the task board itself. So the projection was validated with the multiple cameras (because there were 3 of them). If all 3 cameras detected the task board in the same place within a certain tolerance, that at least confirmed it was detecting it properly. Perhaps a Kalman filter would have been sufficient, but out of all the detections it did, there was maybe 1 in 100 where it saw a different square somewhere else, and when it did detect the right one it was usually accurate to the millimeter, which was definitely good enough!

The port detection itself was done with quite a different technique, namely YOLO. This was the task of my teammate Yara, who handled the whole training framework. Basically she set up the simulator to generate all possible positions of the task board, and retrieved the port locations in those images by using the available ground truth transforms and semantic fill-in and clustering. You should definitely ask her for the specifics! But of course, these detections were only done in 2D on a single camera. However, now that the pose of the task board was known, we could project the task board zones. And if you imagine a ray beaming from the camera through the component, and intersect that beam with the projected zones, then voilà, you have a 3D pose!

## The pre-insertion method

So the task board and component detection solved about 95% of the total movement, namely positioning the arm directly above the port for the trained HIL-SERL policy to take over. In order for the robot to determine the full task board pose, it was necessary for it to rotate the camera into the direction where it could see that binary image (or blob). And that took extra time, plus a risk of the cable getting tangled and stuck.
...
The rules for the qualifying round indicated that the target components were visible in at least one of the cameras attached to the robot arm. So after trying the more elaborate task board detection in the pre-insertion strategy, we went for a hybrid approach. If there were more ports than a certain threshold, it would use the default: rotating, but with a full task board pose detection. If it didn't see as many ports in its view, it would use the simplified one, skipping the task board and going straight from YOLO to the 3D detector.
...
For phase 1 (the round after the qualifying round), we decided to go for a different pre-insertion approach altogether. We were allowed to use Flowstate, the development environment made by Intrinsic, which had a lot of skills available that could take over the initial task board and component detection for us. So we decided to focus on the one element that stayed consistent through the entire competition: the plug insertion policy with HIL-SERL.

## HIL-SERL for the insertion

Anyone who has worked with reinforcement learning knows that it... takes... a... really... long... time! That would mean that we either had to wait weeks for policies to appear, or have a simulator that was able to run a lot of simulations in parallel. Luckily, our teammate Jennifer proposed that we use HIL-SERL, human-in-the-loop sample-efficient reinforcement learning. Setting up that very intricate framework and learning environment was all her hard work, and the initial policies were completely trained by her. I was glad I was able to take some of it over at the end of phase 1.

With reinforcement learning, it's all about developing a policy that determines the 'next right moves' based on the current state. This policy is trained in an environment that has a defined reward function. In our case, the reward was only given when the plug was fully inserted. Of course, getting there completely by chance takes a long time, maybe even never, because that is how chance works, and if it never hits the reward it will simply not learn. So you can either engineer the reward function (the closer it gets to the port, the higher the reward), or have a human show the arm how to do it to reach that reward quicker.

The approach is as follows. You record a set of 20-50 demonstrations, teleoperating the robot arm to insert the plug successfully. After that the training starts, where you have both the learner and the actor active. It will use the demonstrations you recorded before, but it will also take into account any interventions, where you steer the gripper towards directions and orientations where it is more likely to succeed and get that juicy reward. You might even want to do the initial insertion yourself at first, just to increase the chances of it 'accidentally' moving all the way (or at least for the baseline policy trained on the demonstration data). And you can let the robot 'ride without training wheels' for longer and longer as you go along. Here I mainly found that choosing your teleoperation device is extremely important, for which I preferred to use my playstation 5 controller. 
